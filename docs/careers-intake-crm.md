...
| --- | --- | --- |
| `application-id` | alternate key / idempotency | UUID generated by the form and re-used on retry |
...
- upsert by `application-id`; replaying the same ID must return success without creating a duplicate row;
